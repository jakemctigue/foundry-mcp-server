...
import { inspectImageBytes, type JsonValue, type RequestedCapability } from "@foundry-mcp/protocol";
...
import { FakeRole, createRichFakeRuntime } from "../../foundry-module/test/fake-runtime/index.js";
import {
  VALID_PNG,
...
class MockBrowserImageDecoder {
  ...
  readonly createImageBitmap = async (input: unknown): Promise<MockImageBitmap> => {
...
describe("MOCKED FOUNDRY v14 full lifecycle E2E", () => {
  it("survives host/browser/container restarts while covering documents, images, sessions, and intelligence", async () => {
    ...
    runtime.addCompendium({
      ...
      type: "Actor",
      locked: true,
      documents: [{ name: "Compendium Clockwork", type: "clockwork", system: { source: "pack" } }],
    });
    ...
    try {
      const startHost = async (): Promise<void> => {
        ...
        // Identity enrollment intentionally clears stale grants. Grant only
        // after the browser proves or enrolls its durable connection identity.
      ...
      const types = (
        await callTool(client, "foundry.documents.types", {
          connectionId: CONNECTION_ID,
        })
      ...
      expect(imageDecoder.bitmaps).toEqual([expect.objectContaining({ closed: true })]);
      ...
      const clockworkResults = (
        await callTool(client, "foundry.intelligence.search", {
          connectionId: CONNECTION_ID,
          query: "clockwork",
        })
      ).results as unknown[];
      // The actor is LIMITED for the default role, so private-content-off
      // reconciliation excludes its full snapshot while public events remain searchable.
      expect(clockworkResults).toHaveLength(2);
      ...
      ).companion.request(CONNECTION_ID, "documents.create", replayInput, replayRequestId);
